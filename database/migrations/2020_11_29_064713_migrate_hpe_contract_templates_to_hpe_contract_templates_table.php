        ...
        try {
            ...
            DB::table('quote_templates')
                ...
                ->chunk(100, function (Collection $chunk) {
                    ...
                    DB::table('hpe_contract_templates')->insert($rows);
                });
            ...
            $countryPivot = DB::table('country_quote_template')
                ->select('country_id', 'quote_template_id as hpe_contract_template_id')
                ->whereIn('quote_template_id', DB::table('hpe_contract_templates')->select('id'))
                ->get();

            $countryPivot = $countryPivot->map(fn ($pivot) => (array) $pivot)->all();

            DB::table('country_hpe_contract_template')->insert($countryPivot);
        } catch (Throwable $e) {
            DB::rollBack();
