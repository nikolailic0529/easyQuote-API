...
abstract class SchedulesParsingTest extends TestCase
{
     ...
     * @return void
     */
    public function testSchedulesProcessing()
    {
    ...
    {
        $mapping = collect(json_decode(file_get_contents("tests/Unit/Parser/data/schedules/mapping.json"), true));

        $quoteFiles = collect($this->filesList())->map(function ($file) use ($mapping) {
            return $this->user->quoteFiles()->make([
                ...
                'file_type' => __('quote_file.types.schedule'),
                'pages' => $this->parser->countPages($file->getRealPath(), false),
                'imported_page' => data_get(collect($mapping->get($this->countryName()))->firstWhere('filename', '===', $file->getFilename()), 'page'),
    ...
    {
        return Str::before(class_basename($this), 'SchedulesParsingTest');
    }

    protected function filesList()
    {
        $filesPath = "tests/Unit/Parser/data/schedules/{$this->countryName()}";
