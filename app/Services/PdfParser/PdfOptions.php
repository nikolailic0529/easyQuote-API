...
final class PdfOptions
{
    ...
    const REGEXP_PRICE_LINES_NS = '/^(?<product_no>(?!software|hardware|Total|UNITED)\d+\-\w{3}|(?!software|hardware|Total|UNITED)[a-zA-Z]\w{3,12}?[a-zA-Z]{0,2})\s+(?<description>(HPE\h+|.(?![\s\h]{4,}))+[\w\-\+]+)\s{2,}((?<date_from>(([0-2][0-9])|(3[0-1]))[\.\/]((0[0-9])|(1[0-2]))[\.\/]\d{4})\s+?(?=(([0-2][0-9])|(3[0-1]))[\.\/]((0[0-9])|(1[0-2]))[\.\/]\d{4}))?((?<date_to>(([0-2][0-9])|(3[0-1]))[\.\/]((0[0-9])|(1[0-2]))[\.\/]\d{4})\s+?)?(?<qty>\d{1,3}(?=\s+?))(\s+(\p{Sc})?\s?(?<price>(\d{1,3},)?\d+([,\.]\d{1,4})(,\d+)?)(?=\s))(.+)?$/m';
