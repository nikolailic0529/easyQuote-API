...
final class PdfOptions
{
    ...
    const REGEXP_PRICE_LINES_SP = '/^\h*(?!HPE\h{2,})(?<product_no>(?!hardware|Total|UNITED)[\w\-]{4,20})\h{2,15}(?<description>(?:\b[\/\w-]+\b(?: | - |\h{2,5})?)+)\n{1,2}\h{4,}(?:\2\h+(?<date_to>\d{2}\.\d{2}\.\d{4})\h+(?<qty>\d+)\h+(?<price>\d+(?:\'\d*)?\.\d+))?/im';
//    const REGEXP_PRICE_LINES_SP = '/^\h*(?!HPE\h{2,})(?<product_no>(?!hardware|Total|UNITED)[\w\-]+)\h{2,15}(?<description>(\b[\/\w-]+\b ?)+)\n+\h+(?:\2\h+(?<date_to>\d{2}\.\d{2}\.\d{4})\h+(?<qty>\d+)\h+(?<price>\d+\.\d+))?/im'; // original
