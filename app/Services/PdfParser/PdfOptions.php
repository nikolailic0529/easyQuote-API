...
final class PdfOptions
{
    ...
    const REGEXP_SCHEDULE_PAYMENTS = '/\h*(?<payment_dates>((?:system (?:handle|id)|periode de|from|zeitraum\h+von)(\h*:)?)(?<date>(?:[\ha-z-]+)(?:(?:[0-2][0-9])|(?:3[0-1]))[\.\/](?:(?:0[0-9])|(?:1[0-2]))([\.\/]\d{2,4})(?:[\ha-z-]*?))+$)|(^(\h)?(?!payment (\h+)? schedule)(?:period au|to)?(?<payment_dates_options>(\g\'date\')+(?:([\ha-z-]+)?)$))/mi';
