...
use App\Models\{Quote\Quote, QuoteFile\ImportedRawData, QuoteFile\QuoteFile, Quote\FieldColumn};
...
class ParserService implements ParserServiceInterface
{
    ...
    {
        $importedRawDataToArray = function (ImportedRawData $importedRawData): array {
            return [
                'page' => $importedRawData->page,
                'file_path' => $importedRawData->file_path,
            ];
        };

        if ($quoteFile->isSchedule()) {

            with($quoteFile, function (QuoteFile $quoteFile) use ($importedRawDataToArray) {
                $rawData = $quoteFile->importedRawData()
                    ->where('page', '>=', $quoteFile->imported_page)
                    ->orderBy('page')
                    ->get();

                $rawData = array_map($importedRawDataToArray, $rawData->all());

                $paymentData = [];

                foreach ($rawData as $pageData) {
                    $paymentDataFromPage = $this->pdfParser->parseSchedule($pageData);

                    $paymentData = array_merge($paymentData, $paymentDataFromPage);
                }

                DB::transaction(function () use ($paymentData, $quoteFile) {
                    $this->quoteFile->createScheduleData($quoteFile, $paymentData);
        ...
        with($quoteFile, function (QuoteFile $quoteFile) use ($importedRawDataToArray) {
            ...
            $rawData = $quoteFile->importedRawData()->orderBy('page')->get();

            $rawData = array_map($importedRawDataToArray, $rawData->all());

            $parsedData = $this->pdfParser->parse($rawData);

            DB::transaction(function () use ($quoteFile, $parsedData) {
                $this->quoteFile->createRowsData($quoteFile, $parsedData['pages']);
                $quoteFile->storeMetaAttributes($parsedData['attributes']);
                $quoteFile->markAsHandled();
            });
