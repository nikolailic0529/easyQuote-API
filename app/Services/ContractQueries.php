...
class ContractQueries
{
    ...
    public function columnsMappingQuery(Contract $quote)
    {
    ...
    public function searchRowsQuery(Contract $quote, string $search = '', ?string $groupId = null): BaseBuilder
    {
        $inputs = collect(explode(',', $search))->map(fn ($input) => trim($input))->filter()->values();
    ...
    public function mappedSelectedRowsQuery($quote): BaseBuilder
    {
    ...
    public function mappedOrderedRowsQuery(Contract $quote): BaseBuilder
    {
    ...
    public function mappedRowsQuery(Contract $quote): BaseBuilder
    {
        ...
        $defaults->each(
            fn ($value, $column) =>
            $query->unless($mapping->contains('templateField.name', $column), fn (BaseBuilder $query) => $query->selectRaw("{$value} AS `{$column}`"))
        );
