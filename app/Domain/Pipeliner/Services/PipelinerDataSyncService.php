...
use App\Domain\Pipeliner\Services\Models\QueueCounts;
use App\Domain\Pipeliner\Services\Models\QueueSyncResult;
use App\Domain\Pipeliner\Services\RecordCorrelation\RecordCorrelationService;
...
use App\Foundation\Support\Correlation\Contracts\CorrelationAware;
use App\Foundation\Support\Flag\Contracts\FlagsAware;
use Carbon\Carbon;
...
class PipelinerDataSyncService implements LoggerAware, CauserAware, FlagsAware, CorrelationAware
{
    ...
    protected function chainPending(iterable $pending): BaseCollection
    {
                ...
                $currentStrategyChains = $currentStrategyChains
                    ->map(function (BaseCollection $chain) use ($strategyItemsMap, $strategiesToBeChained) {
                        ...
                        $toBeChainedWithCurrent = $strategiesToBeChained
                            ->map(function (string $strategy) use ($item, $strategyItemsMap) {
        ...
        return $chains->map(static function (BaseCollection $chain): BaseCollection {
            return $chain->sortByDesc(static function (array $job): int {
                $item = $job['item'];
                ...
                return Carbon::parse($item['modified'])->roundSeconds()->getTimestamp();
            })->values();
        });
    }
    ...
    public function queueModelSync(Model $model): array
    {
        ...
        $batch = $pendingBatch
            ...
            ->catch(static function (Batch $batch): void {
                /** @var $model Model */
                /** @var $causer \App\Domain\User\Models\User|null */
