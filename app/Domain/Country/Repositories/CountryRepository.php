...
class CountryRepository extends SearchableRepository implements CountryRepositoryInterface
{
    ...
    public function findByCompany(string $company, array $columns = ['*'])
    {
        Country::resolveRelationUsing(
            'companies',
            fn (Country $model) => $model->hasManyDeepFromRelations($model->belongsToMany(Vendor::class), (new Vendor())->companies())
        );
    ...
    {
        return [
            \App\Domain\Country\Queries\Filters\OrderByCreatedAt::class,
            \App\Domain\Country\Queries\Filters\OrderByName::class,
