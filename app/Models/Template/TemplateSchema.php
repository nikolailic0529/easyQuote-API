...
//                ['id' => 'service_agreement_id', 'label' => 'Service Agreement Id'],
    ...
    {
        if (isset(TemplateSchema::$designCache[$name])) {
            return TemplateSchema::$designCache[$name];
        }

        $pages = [];

        foreach (TemplateSchema::DESIGNS[$name] ?? [] as $key => $tags) {
            $pages[$key] = collect($tags)->map(fn ($tag) => $tag + [TemplateSchema::IMAGE_FLAG => false])->toArray();
        }

        return TemplateSchema::$designCache[$name] = $pages;
    }
