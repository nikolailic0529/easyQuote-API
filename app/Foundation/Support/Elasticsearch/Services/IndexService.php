...
use App\Foundation\Support\Elasticsearch\Contracts\HasReindexQuery;
...
class IndexService
{
    ...
    private static function resolveModel(string $class): Model&SearchableEntity
    {
        ...
        if (is_a($class, Model::class, true) === false) {
            throw new \InvalidArgumentException(sprintf('Class [%s] must be an instance of [%s].', $class, Model::class));
        }

        if (is_a($class, SearchableEntity::class, true) === false) {
            throw new \InvalidArgumentException(sprintf('Class must implement [%s] interface.', SearchableEntity::class));
        }
    ...
    public function buildModelIndices(string $class, callable $onStart = null, callable $onProgress = null): void
    {
        ...
        $query->chunk($this->batchSize, static function (Collection $chunk) use ($client, $onProgress): void {
            ...
            foreach ($chunk as $entry) {
                /* @var \App\Foundation\Support\Elasticsearch\Contracts\SearchableEntity $entry */
